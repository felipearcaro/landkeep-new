...
import { classNames, http } from "~/utils";
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 md:flex md:items-center md:justify-between md:space-x-5 lg:max-w-7xl lg:px-8">
        <div className="flex items-center space-x-5">
          ...
          <div>
            ...
            >
              Down payment link:&nbsp;
              <span className="text-blue-700 hover:text-blue-600">{`http://localhost:8082/api/secure/${loan.id}`}</span>
            </Link>
          </div>
        </div>
...
export function ErrorBoundary({ error }: { error: Error }) {
  ...
  let { id } = useParams();
  return (
    <div>{`There was an error loading the loan by the id ${id}. Sorry.`}</div>
  );
}
